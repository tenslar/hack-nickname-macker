...
final class CSVAdapterTest extends HackTest {
    ...
    public function testCreateInstanceWithCSV(
        string $line,
        vec<vec<string>> $expect,
        string $message,
    ): void {
    ...
    public function testCreateInstanceWithEmptyCSV(
        string $unreadableCSVLine,
        string $message,
    ): void {
        ...
        expect(() ==> {
            ...
            \sprintf('CSV file `%s` is empty.', self::TEST_CSV_PATH),
            $message,
        );
